...
percent = StringVar()
bar = Progressbar(window,style='green.Horizontal.TProgressbar',orient='horizontal',length=375,mode='determinate')
bar.pack(pady=20)
...
diff_optimal_ab=10 # Grenze, ab der eine Lösung als 'optimal' gilt (z. B. 10, also 45 % <-> 55 % i.O.)
diff_zulaessig_ab=50 # zulässig ab einer Verteilung von 75 % <-> 25 %

# zu definierende Anzahl Verbesserungsschritte:
max_zufaell_tausche=8 # für Funktion "planverb_zufaellig" - bricht vorher ab, falls alle NB erfüllt und diff_optimal_ab erreicht
max_zufaell_tausche_achslast=12 # ebenfalls für Funktion "planverb_zufaellig"
...
pss_pfad = '.../Testdaten November 2022/Daten vor Stauraumplanung/pss.asc'
...
for z_blk in blk_vor: # nicht in späteres identisches for integriert, weil ich dies nicht für jede z_tar in tar_vor wiederholen möchte
    ...
    if z_blk[14:16] == 'LK': # ansonsten ist die Tour von uns nicht zu berücksichtigen
...
for z_tar in tar_vor:
    if z_tar[14:16] == 'AR': # ansonsten ist die Tour von uns nicht zu berücksichtigen
        ...
        if (vorgaben_spediteure.loc[:, 'Spediteur'] == z_tar[53:63].strip()).any(): # wenn es den Spediteur aus tar in unseren Spediteursvorgaben gibt
            ...
            # und gib es zum Schluss als Hinweis in der GUI aus
            if f'Spediteur {z_tar[53:63].strip()} unbekannt. Defaultwert "Andere" gesetzt' not in fazit_gui:
                fazit_gui.append(f'Spediteur {z_tar[53:63].strip()} unbekannt. Defaultwert "Andere" gesetzt')
        ...
        for z_blk in blk_vor:
            if z_blk[426:431] == z_tar[18:23]:  # gleiche Tournr.
                ...
                usp = z_blk[54:60].strip()
                if usp == '0': # Usp 0 ist nichtssagend, hier Auslieferungsreihenfolge der Kundennr. nutzen
                    # auch hinter einer Kundennr. bei Usp 0 können mehrere Auslieferungsreihenfolgenr. stecken - später auch hier Minimum wählen
                    usp = 'kd'+(z_blk[46:53].strip()) # Spalte Kunde in BLK
                    # Kennzeichnung kd benötigt, da es z. B. Kundennr. 5 gibt, die NICHT Umschlagspunkt 5 entspricht
...
for idx in range(0,anzahl_touren): # nicht in vorherige for-Schleife integrierbar, weil wir blkblo_je_tour benötigen
    ...
    for z_pss in pss_vor:
        ...
        if z_pss[25:32] in blkblo_je_tour[idx]['Auftragsnr']: # wenn BLKNR zur aktuell bearbeiteten Tour gehört
            ...
            if blkblo_blktre_blkk02[z_pss[25:32]][1] == 'Y' and int(z_pss[110:111]) != 2:
                # wenn Kennz.Kommission/Klima=Y, da der Kühlstatus für uns nur dann relevant ist
                # wollen nur den originalen Eintrag, wenn dieser nicht 2 ist
...
def plan_fuellen(tabelle, lkwplan, anz_pal, oben, option):
    if option == 1:  # "normal", aufsteigend durch Zeilen und Spalten
        ...
        while idx_pal < anz_pal:
            ...
            if row_b == 0 and tabelle.loc[idx_pal:idx_pal + 5, 'h_i'].sum() >= 1:  #(.loc[0:2] umfasst Zeilen 0,1,2 - .iloc[0:2] nur Zeilen 0 und 1)
    ...
    for col in range(0, lkwplan_gew.shape[1]):
        for row in range(0, lkwplan_gew.shape[0]):
            if type(lkwplan_gew.iloc[row, col]) != str:
                ...
                lkwplan_gew.iloc[row, col] = 0  # Alternative hierzu wäre z. B. lkwplan_gewichte.replace('x',0,inplace=True)
    ...
    if anz_pal - bewertung[0] <= 27 or (anz_pal - bewertung[0] >= 34 and anz_pal - bewertung[0] <= 60):
        ...
        while col_idx < lkwplan_.shape[1]:
            ...
            try:
                ...
                else:  # wenn nicht nur Zahlen oder nicht nur x, kann man aufhören, da Restreihe gefunden
                    if col_idx < 10:
                        typwechsel += 1
                    col_idx = 11
                    break
...
def gewichte_optimieren(tabelle_, lkwplan_, bewertung_):
    ...
    for col in range(0, plan_tauschpotential.shape[1]):
        ...
        for row in rows_potential:
            if row == 0 or row == 3:
                plan_tauschpotential.iloc[row, col] = (plan_gewichte.iloc[row, col] - plan_gewichte.iloc[row + 1, col]) * w
            elif row == 1 or row == 4:
                plan_tauschpotential.iloc[row, col] = (plan_gewichte.iloc[row - 1, col] - plan_gewichte.iloc[row + 1, col]) * w
            elif row == 2 or row == 5:
                plan_tauschpotential.iloc[row, col] = (plan_gewichte.iloc[row - 1, col] - plan_gewichte.iloc[row, col]) * w
    ...
    # finde die Reihe mit dem Wert am nächsten zur aktuellen Gewichtsdifferenz diff (um möglichst optimal wegzutauschen)
            ...
            if sp_sum[idx] - sp_sum_oben[idx] <= 2000:  # dann tausche oben und unten komplett
                ...
                for a in [1, 2]:  # tausche schwerste oben mit leichtester unten (2x)
                    ...
                    if row_max > row_min:
                        ...
                        lkwplan_.iloc[row_min, idx] = pd.DataFrame(lkwplan_.iloc[row_max, idx].copy(deep=True)).iloc[:,0]
                        lkwplan_.iloc[row_max, idx] = pd.DataFrame(speicher.copy(deep=True)).iloc[:, 0]

    return lkwplan_


def tauschpartner_andere_reihe(lkwplan_, lkwplan_n_werte, lkwplan_bew, tabelle_2, pal_row_idx, pal_col_idx, col_idx_neu, n_wert):
    tauschpartner = []  # schreibe eine Liste mit potenziellen Tauschpartnern als [row,col]-ELemente
    tauschpartner_achslast = []  # und ggf. eine Liste mit potenziellen Tauschpartnern für Achslasttausche
    ...
    # hier berücksichtigen wir den t_wert nicht, da dieser ggf. vernachlässigt werden darf & Bewertung das bei Suche der besten Lsg berücksichtigt
    m_wert = tabelle_2.loc[lkwplan_.iloc[pal_row_idx, pal_col_idx], 'm_i']
    ...
    return tauschpartner, tauschpartner_achslast


def planverb_zufaellig(tabelle_, lkwplan_, lkwplan_bew, max_tausche_zufall, max_tausche_achslast):
    ...
    while tauschnr_zufall + tauschnr_achslast < max_tausche_zufall + max_tausche_achslast:
        # zufällige Palette auswählen
        pal_row_idx = randint(0, 5)  # nur bis 5, da wir den Index nutzen und randint(0,2) von 0 bis einschl. 2 zieht

        if max_tausche_achslast > 0:  # dann lieber die Randreihen betrachten, weil das voraussichtlich mehr bringt
            pal_col_idx = choice([0, 1, 2, 8, 9, 10])
        else:
            pal_col_idx = randint(0, 10)
        ...
        if n_wert != 'x':
            ...
            if pal_col_idx > 0:
                ...
                liste, liste_achslast = tauschpartner_andere_reihe(lkwplan_a, lkwplan_n_werte_a, lkwplan_a_bew,
                                                                   tabelle_2, pal_row_idx, pal_col_idx,
                                                                   col_idx_neu, n_wert)
                tauschpartner.extend(liste)
                tauschpartner_achslast.extend(liste_achslast)
            ...
            if tauschnr_achslast < max_tausche_achslast and len(tauschpartner_achslast) > 0:
                # zufälligen Tauschpartner aus Potentialliste Achslast bestimmen
                randidx = randint(0, len(tauschpartner_achslast) - 1)
                tauschelement = tauschpartner_achslast[randidx]
                tauschnr_achslast += 1

            elif tauschnr_zufall < max_tausche_zufall:
                # zufälligen Tauschpartner aus Gesamtliste bestimmen
                randidx = randint(0, len(tauschpartner) - 1)
                tauschelement = tauschpartner[randidx]
                tauschnr_zufall += 1

            else:
                break

            if type(tauschelement[0]) == int:
                lkwplan_v.iloc[pal_row_idx, pal_col_idx] = lkwplan_a.iloc[tauschelement[0], tauschelement[1]]
                lkwplan_v.iloc[tauschelement[0], tauschelement[1]] = lkwplan_a.iloc[pal_row_idx, pal_col_idx]

                lkwplan_n_werte_v.iloc[pal_row_idx, pal_col_idx] = lkwplan_n_werte_a.iloc[
                    tauschelement[0], tauschelement[1]]
                lkwplan_n_werte_v.iloc[tauschelement[0], tauschelement[1]] = lkwplan_n_werte_a.iloc[
                    pal_row_idx, pal_col_idx]

            else:
                # Dreiertausch benötigt: A=Ausgangspal, B=gleiches n, C=anderes n (bleibt in der Reihe)
                # setze A_a auf C_v
                # setze B_a auf A_v
                # setze C_a auf B_v

                lkwplan_v.iloc[tauschelement[1][0], tauschelement[1][1]] = lkwplan_a.iloc[pal_row_idx, pal_col_idx]
                lkwplan_v.iloc[pal_row_idx, pal_col_idx] = lkwplan_a.iloc[tauschelement[0][0], tauschelement[0][1]]
                lkwplan_v.iloc[tauschelement[0][0], tauschelement[0][1]] = lkwplan_a.iloc[
                    tauschelement[1][0], tauschelement[1][1]]

                lkwplan_n_werte_v.iloc[tauschelement[1][0], tauschelement[1][1]] = lkwplan_n_werte_a.iloc[
                    pal_row_idx, pal_col_idx]
                lkwplan_n_werte_v.iloc[pal_row_idx, pal_col_idx] = lkwplan_n_werte_a.iloc[
                    tauschelement[0][0], tauschelement[0][1]]
                lkwplan_n_werte_v.iloc[tauschelement[0][0], tauschelement[0][1]] = lkwplan_n_werte_a.iloc[
                    tauschelement[1][0], tauschelement[1][1]]
            ...
            if math.floor(lkwplan_v_bew[1]) + math.floor(lkwplan_v_bew[2]) + sum(lkwplan_v_bew[4:6]) <= 0.001 and \
                ...
                # wenn noch nicht gut genug aber besser als vorher, wollen wir das als neues Ergebnis speichern
...
def planauswahl_final(tabelle, planliste, planliste_v2, planliste_v3, diff_optimal_ab, max_tausche,
    ...
    if len(planliste_v2) > 0:
        ...
        bester_pl_v2, bester_pl_v2_bew = planauswahl_final(tabelle, planliste_v2, planliste_v3, [], diff_optimal_ab,
                                                           max_tausche, max_tausche_achslast)
...
for tour_idx in range(0, anzahl_touren):
    ...
    percent.set(f'     Tour {tour_idx + 1}/{anzahl_touren} in Bearbeitung     ')
    window.update_idletasks()
    ...
    if len(tour_laender) == 1:
        ...
        m_HA_max_ges = laendervorgaben.loc[min(tour_laender), 'Achslast Sattelzug Hinten']  # min gibt hier ein Element der Menge zurück (als Startwert)
        m_t_max_ges = laendervorgaben.loc[min(tour_laender), 'Auflieger Dreiachsig']
    ...
    if anz_pal > vorgaben_spediteure.loc[tarnr_tarspd[tour_idx][1], 'Gesamtkapazitaet']:
        # Spediteursvorgaben zur maximalen Palettenanz. (entweder Doppelstock=66 oder nicht=33) überschritten?
        warnungen[tour_idx].append('Zu viele Paletten für diesen Spediteur eingeplant!')

    elif m_lad > vorgaben_spediteure.loc[tarnr_tarspd[tour_idx][1], 'Zulaessige Nutzlast']:
        # Spediteursvorgaben zur maximalen Nutzlast überschritten?
    ...
    Ende Vorarbeit
    Beginn des tatsächlichen Verfahrens
    zunächst Multi-Startverfahren
    ______________________________________________________________________________
    """

    # man könnte Verfahren auch nur durchlaufen, wenn keine Warnung erzeugt wurde
    # if len(warnungen[tour_idx])==0:
    # wir durchlaufen es aber trotzdem & geben hinterher sowieso Feedback an den User

    varianten = {'A': {'by': ['n_i', 't_i'], 'ascending': [False, True]},
                 'B': {'by': ['n_i', 't_i'], 'ascending': [False, True]},
                 'C': {'by': ['n_i', 't_i'], 'ascending': [False, True]},
                 'D': {'by': ['n_i', 't_i', 'm_i'], 'ascending': [False, True, False]},
                 'E': {'by': ['n_i', 't_i', 'm_i'], 'ascending': [False, True, True]},
    ...
    # S3 bis S6 beschreiben zufällige Entscheidungen je n
    ...
    for var in varianten:
        ...
        elif var == 'S1' or var == 'S2':
            for nr in range(0, anz_n):
                n_einzeln = liste_n[nr]

                rest = (0 if var == 'S1' else 1)
                # ändert, ob HPal von 1&2 beieinander stehen oder von 2&3 usw.

                if nr % 2 == rest:  # ==0 für jedes zweite n_nr
                ...
                if nr == 0:  # im ersten Durchlauf zunächst tabelle_alle erzeugen
        ...
        planoption = 1
        anz_opt_plan_fuellen = 3  # Optionen in Funktion plan_fuellen(...,option)
        while planoption <= anz_opt_plan_fuellen:
            ...
            # ermöglicht Zeitersparnis, aber auch ggf. etwas schlechtere (dennoch als gut geltende) Lösungen
    ...
    if len(plaene) > 0 or len(plaene_zweitewahl) > 0:
        plan_final, plan_final_bew = planauswahl_final(tabelle, plaene, plaene_zweitewahl, plaene_pal_ueber,
                                                       ...
                                                       max_zufaell_tausche_achslast)
    else:
        plan_final, plan_final_bew = planauswahl_final(tabelle, plaene_pal_ueber, [], [], diff_optimal_ab,
                                                       max_zufaell_tausche, max_zufaell_tausche_achslast)

    if sum(plan_final_bew[0:6]) <= 0.001 and plan_final_bew[-1] <= diff_optimal_ab:
        fazit[
            tour_idx] = f'Gute Lösung gefunden, Gewichtsdifferenz beträgt {plan_final_bew[-1]: .2f} %  -  entspricht ca. {plan_final_bew[-1] / 100 * sum(tabelle["m_i"]): .2f} kg Mehrgewicht auf einer Seite'
    ...
    elif sum(plan_final_bew[0:2]) + sum(plan_final_bew[4:6]) <= 0.001 and plan_final_bew[-1] <= diff_zulaessig_ab:
        ...
        if plan_final_bew[0]>1:
            fazit_gui.append(f'Tour {tarnr_tarspd[tour_idx][0]}:   {plan_final_bew[0]} Paletten konnten nicht eingeplant werden')
        elif plan_final_bew[0]>0:
            fazit_gui.append(f'Tour {tarnr_tarspd[tour_idx][0]}:   {plan_final_bew[0]} Palette konnte nicht eingeplant werden')
    ...
    for col in range(0, 11):
        for row in [1, 0, 2, 4, 3, 5]:  # andere Sortierung der Reihen hilfreich für Anpassung der Ladereihenfolge
            ...
            if type(wert) != str:
                # ladenr += 1 # hier hochzählen, falls Nr. freier Plätze doch nicht übersprungen werden soll (1,2,3,x,x,x,4,5)
                psszeile = tabelle.loc[wert, 'zeile']
                gesamtzeilenliste.remove(psszeile)

                # Tournummer
                pss_vor[psszeile] = pss_vor[psszeile][:94] + "00" + str(tarnr_tarspd[tour_idx][0]) + pss_vor[psszeile][101:]

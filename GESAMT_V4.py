...
def tauschpartner_andere_reihe(lkwplan_, lkwplan_n_werte, lkwplan_bew, tabelle_2, pal_row_idx, pal_col_idx, col_idx_neu,
                               n_wert):
    ...
    if len(planliste) > 0:
        ...
        for pl in planliste:
            ...
            if sum(pl[1][0:6]) <= 0.001 and pl[1][-1] <= diff_optimal_ab:
                ...
                if pl[1][0] > 0 and pl[1][0] >= planliste[0][1][
                    0]:  # dann gibt es eine Lsg mit akzeptabler Bewertung, weil plaene_pal_ueber extra so sortiert
                    # hier sparen wir uns ansonsten aufwändige Wiederholungen für Achslastverbesserungen
                    pl[0], pl[1] = planverb_zufaellig(tabelle, pl[0], pl[1], max_tausche,
                                                      0)  # beinhaltet auch gewichte_optimieren
                else:
                    pl[0], pl[1] = planverb_zufaellig(tabelle, pl[0], pl[1], max_tausche,
                                                      max_tausche_achslast)  # beinhaltet auch gewichte_optimieren
